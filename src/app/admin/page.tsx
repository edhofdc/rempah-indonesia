            ...
            {!data?.lowStock || data.lowStock.length === 0 ? (
              ...
              <div className="space-y-2">
                {data.lowStock.map((product) => (
                  ...
                  >
                    <div>
                      <p className="text-sm font-medium text-brown-800">{product.name}</p>
                ...
                {data.recentOrders.map((order) => (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-green-600">{order.orderNumber}</p>
                      <p className="text-xs text-green-500 mt-0.5">
                        {order.items.length} item(s) &middot; {formatPrice(order.subtotal)}
                      </p>
                    </div>
                    <div className="text-right flex-shrink-0 ml-3">
                      ...
                      <p className="text-[10px] text-green-400 mt-1">{formatDate(order.createdAt)}</p>
                    </div>
                  </div>
                ))}
              </div>
