...
export default function AdminLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-green-50">
      ...
      <aside
        className={`fixed top-0 left-0 z-50 h-full bg-gradient-to-b from-green-900 via-green-800 to-green-950 text-white transition-all duration-300 shadow-2xl ${sidebarOpen ? "w-64" : "w-16"} ${mobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
        <div className="flex items-center gap-3 px-4 h-16 border-b border-green-700/50">
          ...
          <div
            className={`overflow-hidden transition-all duration-300 ${sidebarOpen ? "opacity-100 w-auto" : "opacity-0 w-0"}`}
          >
            <h1 className="text-sm font-bold text-cream-100 whitespace-nowrap">
              Rempah Indonesia
            </h1>
          ...
          <button
            ...
            className="ml-auto hidden lg:flex items-center justify-center w-6 h-6 rounded-md hover:bg-green-700/50 transition-colors"
          >
            {sidebarOpen ? (
        ...
        <header className="h-16 bg-white border-b border-green-200 shadow-sm flex items-center justify-between px-4 lg:px-6 sticky top-0 z-30">
          <div className="flex items-center gap-3">
            <button
              ...
              className="lg:hidden p-2 rounded-lg hover:bg-green-50 text-green-700"
            >
